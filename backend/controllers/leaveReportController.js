...
const buildLeaveReportPdf = async (payload) => {
  try {
    ...
    // Summary Statistics - 4 cards in grid
    ...
    // Employee Leave Information - table (first 30)
    if (employeeLeaveInfo && employeeLeaveInfo.length > 0) {
      page.drawText('Employee Leave Information (First 30)', {
        x: 50,
        y: y,
        size: 16,
        font: fontBold,
      ...
      page.drawText('Dept', { x: 140, y: y, size: 9, font: fontBold, color: primaryColor });
      page.drawText('Leave Type', { x: 200, y: y, size: 9, font: fontBold, color: primaryColor });
      page.drawText('Days', { x: 300, y: y, size: 9, font: fontBold, color: primaryColor });
      page.drawText('Remaining', { x: 360, y: y, size: 9, font: fontBold, color: primaryColor });
      page.drawText('Status', { x: 440, y: y, size: 9, font: fontBold, color: primaryColor });

      y -= 15;

      employeeLeaveInfo.slice(0, 30).forEach((emp) => {
        const empName = (emp.employee_name || 'N/A').substring(0, 18);
        const department = (emp.department || 'N/A').substring(0, 10);
        const leaveType = (emp.leave_type || 'N/A').substring(0, 15);
        ...
        page.drawText(department, { x: 140, y: y, size: 8, font: font, color: textColor });
        page.drawText(leaveType, { x: 200, y: y, size: 8, font: font, color: textColor });
        page.drawText(String(daysTaken), { x: 300, y: y, size: 8, font: font, color: textColor });
        page.drawText(String(remaining), { x: 360, y: y, size: 8, font: font, color: textColor });
        page.drawText(status, { x: 440, y: y, size: 8, font: font, color: textColor });
