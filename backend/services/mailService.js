...
const sendPayslipGeneratedEmail = async ({ toEmail, toName, period, generatedBy }) => {
  ...
  await sendBrevoEmail({
    ...
    subject: `Your payslip for ${periodLabel} is now available`,
    htmlContent: `
      <div style="margin: 0; background: #f0fdf4; padding: 28px; font-family: Arial, sans-serif; color: #0f172a; line-height: 1.55;">
        <div style="margin: 0 auto; max-width: 640px;">
          <p style="margin: 0 0 12px; color: #166534; font-weight: 700;">Payslip Available</p>
          <h1 style="margin: 0; font-size: 28px; color: #052e16;">Your payslip for ${periodLabel} is ready</h1>
          <p style="margin: 12px 0 0; color: #475569;">Hello ${toName || 'there'}, your payslip for ${periodLabel} has been generated and is now available in KEREA HRMS.</p>
          <div style="margin: 24px 0; overflow: hidden; border-radius: 20px; border: 1px solid #d1fae5; background: #ffffff; box-shadow: 0 18px 45px rgba(20,83,45,0.12);">
            <div style="background: linear-gradient(135deg, #14532d, #22c55e); padding: 18px 22px; color: #ffffff;">
              <p style="margin: 0; font-size: 12px; letter-spacing: 0.14em; text-transform: uppercase; opacity: 0.82;">KEREA HRMS Payroll</p>
              <h2 style="margin: 6px 0 0; font-size: 22px; line-height: 1.25;">Payslip Details</h2>
            </div>
            <div style="padding: 20px 22px; color: #0f172a;">
              <p style="margin: 0 0 10px;"><strong>Period:</strong> ${periodLabel}</p>
              <p style="margin: 0 0 10px;"><strong>Generated by:</strong> ${generatedBy || 'Finance Team'}</p>
              <p style="margin: 0;">Please log in to HRMS to view and download your payslip.</p>
            </div>
          </div>
          <div style="margin: 24px 0;">
            <a href="${String(env.frontendUrl || '').replace(/\/+$/, '')}/payslips" style="display: inline-block; padding: 13px 22px; background: #166534; color: #ffffff; text-decoration: none; border-radius: 14px; font-weight: 700;">
              View Payslip in HRMS
            </a>
          </div>
          <p style="margin: 0; color: #475569;">If you have any questions about your payslip, please contact the finance department.</p>
        </div>
      </div>
    `
  });
...
const sendTimesheetSubmittedEmail = async ({ toEmail, toName, employeeName, period, timesheetId }) => {
  await sendBrevoEmail({
    to: [
      {
        email: toEmail,
    ...
    subject: `Timesheet submitted by ${employeeName} for ${period}`,
    htmlContent: `
      <div style="margin: 0; background: #eff6ff; padding: 28px; font-family: Arial, sans-serif; color: #0f172a; line-height: 1.55;">
        <div style="margin: 0 auto; max-width: 640px;">
          ...
          <h1 style="margin: 0; font-size: 28px; color: #1e3a8a;">Timesheet Submitted for Review</h1>
          <p style="margin: 12px 0 0; color: #475569;">${employeeName} has submitted their timesheet for ${period} and is awaiting your approval.</p>
          <div style="margin: 24px 0; overflow: hidden; border-radius: 20px; border: 1px solid #dbeafe; background: #ffffff; box-shadow: 0 18px 45px rgba(30,64,175,0.12);">
            <div style="background: linear-gradient(135deg, #1e3a8a, #3b82f6); padding: 18px 22px; color: #ffffff;">
              <p style="margin: 0; font-size: 12px; letter-spacing: 0.14em; text-transform: uppercase; opacity: 0.82;">KEREA HRMS Timesheet</p>
              <h2 style="margin: 6px 0 0; font-size: 22px; line-height: 1.25;">Timesheet Details</h2>
            </div>
            <div style="padding: 20px 22px; color: #0f172a;">
              <p style="margin: 0 0 10px;"><strong>Employee:</strong> ${employeeName}</p>
              <p style="margin: 0 0 10px;"><strong>Period:</strong> ${period}</p>
              <p style="margin: 0;">Please review and approve or reject this timesheet.</p>
            </div>
          </div>
          <div style="margin: 24px 0;">
            <a href="${String(env.frontendUrl || '').replace(/\/+$/, '')}/timesheets" style="display: inline-block; padding: 13px 22px; background: #1e40af; color: #ffffff; text-decoration: none; border-radius: 14px; font-weight: 700;">
              Review Timesheet in HRMS
            </a>
          </div>
          <p style="margin: 0; color: #475569;">Please log in to HRMS to review this timesheet.</p>
        </div>
      </div>
    `
  });
...
const sendTimesheetApprovedEmail = async ({ toEmail, toName, period, approvedBy }) => {
  await sendBrevoEmail({
    to: [
      {
        email: toEmail,
    ...
    subject: `Your timesheet for ${period} has been approved`,
    htmlContent: `
      <div style="margin: 0; background: #f0fdf4; padding: 28px; font-family: Arial, sans-serif; color: #0f172a; line-height: 1.55;">
        <div style="margin: 0 auto; max-width: 640px;">
          <p style="margin: 0 0 12px; color: #166534; font-weight: 700;">Approved</p>
          <h1 style="margin: 0; font-size: 28px; color: #052e16;">Your Timesheet Has Been Approved</h1>
          <p style="margin: 12px 0 0; color: #475569;">Hello ${toName || 'there'}, your timesheet for ${period} has been approved by ${approvedBy}.</p>
          <div style="margin: 24px 0; overflow: hidden; border-radius: 20px; border: 1px solid #d1fae5; background: #ffffff; box-shadow: 0 18px 45px rgba(20,83,45,0.12);">
            <div style="background: linear-gradient(135deg, #14532d, #22c55e); padding: 18px 22px; color: #ffffff;">
              <p style="margin: 0; font-size: 12px; letter-spacing: 0.14em; text-transform: uppercase; opacity: 0.82;">KEREA HRMS Timesheet</p>
              <h2 style="margin: 6px 0 0; font-size: 22px; line-height: 1.25;">Approval Details</h2>
            </div>
            <div style="padding: 20px 22px; color: #0f172a;">
              <p style="margin: 0 0 10px;"><strong>Period:</strong> ${period}</p>
              <p style="margin: 0 0 10px;"><strong>Approved by:</strong> ${approvedBy}</p>
              <p style="margin: 0;">Your timesheet has been successfully approved and recorded.</p>
            </div>
          </div>
          <div style="margin: 24px 0;">
            <a href="${String(env.frontendUrl || '').replace(/\/+$/, '')}/timesheets" style="display: inline-block; padding: 13px 22px; background: #166534; color: #ffffff; text-decoration: none; border-radius: 14px; font-weight: 700;">
              View Timesheet in HRMS
            </a>
          </div>
...
const sendTimesheetRejectedEmail = async ({ toEmail, toName, period, rejectedBy, comment }) => {
  await sendBrevoEmail({
    to: [
      {
        email: toEmail,
